...
// Seed token data in case Bankr API is blocked
const SEED_TOKENS = [
...
export async function GET() {
  // Retry live Bankr API with different strategies
  ...
  try {
    ...
    if (res && res.ok) {
      ...
      const tokens = raw
        ...
        .filter((t: { deployed_at: string }) => {
          if (!t.deployed_at) return false;
          const d = new Date(t.deployed_at.replace(" ", "T") + "Z");
          return !Number.isNaN(d.getTime()) && d >= cutoff;
        });

      const alerts = tokens.filter((t: { vol_24h: number }) => t.vol_24h >= 50000);

      return NextResponse.json({
        generated_at: new Date().toISOString(),
        engine: "NyxScout (live)",
        summary: {
          total_scanned: raw.length, fresh: tokens.length,
          running: tokens.filter((t: { vol_24h: number }) => t.vol_24h >= 10000).length,
          new_tokens: tokens.filter((t: { deployed_at: string }) => {
            const d = new Date(t.deployed_at.replace(" ", "T") + "Z");
            return !Number.isNaN(d.getTime()) && (now.getTime() - d.getTime()) < 86400000;
          }).length,
          alerts: alerts.length, top_score: Math.max(...tokens.map((t: { score: number }) => t.score)),
        },
        tokens, alerts,
      });
    }

    // Fallback: seed data (live API unavailable)
    return NextResponse.json({
      generated_at: new Date().toISOString(),
      engine: "NyxScout (seed data — live API unreachable from Railway)",
      summary: {
        total_scanned: 500, fresh: SEED_TOKENS.length,
        running: SEED_TOKENS.filter((t) => t.vol_24h >= 10000).length,
        new_tokens: SEED_TOKENS.length, alerts: SEED_TOKENS.filter((t) => t.vol_24h >= 50000).length,
        top_score: Math.max(...SEED_TOKENS.map((t) => t.score)),
      },
      tokens: SEED_TOKENS,
      alerts: SEED_TOKENS.filter((t) => t.vol_24h >= 50000),
    });

  } catch {
    return NextResponse.json({
      generated_at: new Date().toISOString(),
      engine: "NyxScout (seed fallback)",
      summary: { total_scanned: 500, fresh: SEED_TOKENS.length, running: SEED_TOKENS.length, new_tokens: SEED_TOKENS.length, alerts: SEED_TOKENS.filter((t) => t.vol_24h >= 50000).length, top_score: Math.max(...SEED_TOKENS.map((t) => t.score)) },
      tokens: SEED_TOKENS,
      alerts: SEED_TOKENS.filter((t) => t.vol_24h >= 50000),
    });
  }
